...
import { setToken } from "@/store/sessionsSlice";
...
const Header = () => {
  ...
  const getInitials = () => {
    if (user) {
      ...
      return (
        name[0].charAt(0).toUpperCase() + lastName[0].charAt(0).toUpperCase()
      );
    }
  };
      ...
      <Toolbar sx={{ display: "flex", justifyContent: "space-between" }}>
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="text.primary">
            {drawerItems.find((item) => item.path === location.pathname)
              ?.label || "Usuarios"}
          </Typography>
        </Breadcrumbs>
        ...
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          >
            <Tooltip title="Cuenta">
              ...
              >
                <Avatar sx={{ width: 32, height: 32 }}>{getInitials()}</Avatar>
              </IconButton>
            </Tooltip>
          </Box>
        </Box>
